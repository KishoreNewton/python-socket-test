    ...
    def handle(self):
        ...
        while True:
            try:
                ...
                self.process_request(client_data)

            except socket.timeout:
                logging.info("Connection timed out")
                break
            except UnicodeDecodeError:
                logging.warning("Received data is not valid utf-8 encoded")
                self.request.sendall(b'Invalid data encoding')
            except Exception as e:
                logging.error(f"An error occurred: {e}")
                self.request.sendall(b'An error occurred')
